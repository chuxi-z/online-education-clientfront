  ...
  <div id="aCoursesList" class="bg-fa of">
    ...
    <section class="container">
      ...
      <section class="c-sort-box unBr">
        <div>
          <!-- /无数据提示 开始-->
          <section class="no-data-wrap" v-if="teacherList.total == 0">
            <em class="icon30 no-data-ico">&nbsp;</em>
            <span class="c-666 fsize14 ml10 vam">没有相关数据，小编正在努力整理中...</span>
          </section>
          <!-- /无数据提示 结束-->
          <article class="i-teacher-list" v-if="teacherList.total > 0">
            <ul class="of">
              <li v-for="teacher in teacherList.records" :key="teacher.id">
        ...
        <div>
          <div class="paging">
            ...
            <a
              ...
              href="#"
              title="末页"
              @click.prevent="gotoPage(teacherList.pages)">末</a>
            <div class="clear"/>
          </div>
        </div>
        <!-- 公共分页 结束 -->
...
export default {
  ...
  asyncData({params, error}){
    return teacherApi.getTeacherPage(1, 8)
      .then(response =>{
        return { teacherList: response.data.data }
      })
  },
  methods: {
    gotoPage(page){
